...
from app import Barber, Customer, Barbershop, Service, Appointment
...
def test_barber_customer_appointment_flow(client):
    # Barber signs up
    ...
    # Customer updates the appointment
    appointment = Appointment.query.filter_by(customer_id=customer.id).first()
    response = client.get(f'/update_appointment/{appointment.id}', follow_redirects=True)
    assert response.status_code == 200
    assert b"Update Appointment" in response.data

    response = client.post(f'/update_appointment/{appointment.id}', data=dict(
        start_time="10:00"
    ), follow_redirects=True)
    assert response.status_code == 200
    assert b"Appointment updated successfully." in response.data

    # Customer deletes the appointment
